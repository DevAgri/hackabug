<template>
  <div id="Plantio" class="black--text">
    ...
    <v-card>
        ...
        <v-btn @click.native="muda()" v-show="!visible" floating small success>
          ...
          <form v-on:submit.prevent="adiciona()">
            <v-text-field
              ...
              single-line
              required
              v-model="plantio.tecnologia"
            ></v-text-field>
            <v-select
              label="Selecione a cultura"
              v-bind:items="culturas"
              v-model="plantio.cultura"
              item-text="nome"
              item-value="id"
              single-line
              max-height="auto"
              autocomplete
            ></v-select>
            <v-date-picker v-model="plantio.dataPlantio"></v-date-picker>
            <v-spacer></v-spacer>
      ...
      >
        ...
        <template slot="items" scope="props">
          ...
          <td class="text-xs-left">{{ props.item.dataPlantio | formatedDate }}</td>
          <td class="text-xs-left">{{ props.item.cultura.nome }}</td>
...
import PlantioService from '../../domain/plantio/PlantioService'
import Plantio from '../../domain/plantio/Plantio'
import CulturaService from '../../domain/cultura/CulturaService'
export default {
  name: 'Home',
  data () {
    return {
  ...
  created () {
    let culturaService = new CulturaService(this.$http)
    culturaService.load()
    .then(function (res) {
      this.culturas = res
    ...
    this.service = new PlantioService(this.$http)
    this.service.load()
    .then(function (res) {
      console.log(res)
      this.items = res
  ...
  methods: {
    ...
    adiciona () {
      this.isBusy = true
      this.service.add(this.plantio)
      .then(function (res) {
        this.warningMessage = 'Plantio Adicionado com sucesso'
        this.items.add(this.plantio)
    ...
    removeSelecionados () {
      this.selected.forEach(function (element) {
        ...
        .then(function (res) {
          ...
          console.log(err)
          this.warningMessage = 'Houve um erro ao excluir a(s) Plantio(s)'
        })
      }, this)
    }
  }
